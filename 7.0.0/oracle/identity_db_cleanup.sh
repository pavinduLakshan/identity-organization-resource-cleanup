...
AVAILABLE_TABLES=$(echo "SELECT table_name FROM user_tables;" | sqlplus -s "$DB_USER/$DB_PASSWORD@//$DB_HOST:$DB_PORT/$DB_NAME" | grep -v "^$" | tr '[:lower:]' '[:upper:]')
...
COMMIT;
EXIT;
" | sqlplus -s "$DB_USER/$DB_PASSWORD@//$DB_HOST:$DB_PORT/$DB_NAME" 2>&1)
    SQLPLUS_RC=$?
    ...
    if [[ $SQLPLUS_RC -eq 0 ]]; then
      ...
      { echo "--- SQL ---"; echo "$DELETE_PROCEDURE"; echo "--- Output ---"; echo "$SQLPLUS_OUTPUT"; } > "$FAILED_LOG"
