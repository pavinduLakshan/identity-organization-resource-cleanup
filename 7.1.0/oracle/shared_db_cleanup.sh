...
# Step 2: Get available tables from Oracle database
echo "Fetching available tables from Oracle database..."
AVAILABLE_TABLES=$(echo "SET HEAD OFF; SET FEEDBACK OFF; SET PAGESIZE 0; SELECT table_name FROM user_tables;" | sqlplus -s "$DB_USER/$DB_PASSWORD@//$DB_HOST:$DB_PORT/$DB_NAME" | grep -v "^$" | tr '[:lower:]' '[:upper:]')
...
while IFS=',' read -r TENANT_ID ORG_UUID; do
  ...
  if [[ -n "$TENANT_ID" && -n "$ORG_UUID" ]]; then
    ...
    echo "$DELETE_PROCEDURE" | sqlplus -s "$DB_USER/$DB_PASSWORD@//$DB_HOST:$DB_PORT/$DB_NAME" > /dev/null 2>&1
